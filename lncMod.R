lncMod <- function (md, CP, M.exp, E.exp, T.exp, WS = 0.25, method = "spearman",iqr.filter = c(log2(1.5), log2(1.5), log2(1.5)), cor.MvsCP = c(-0.3,0.3), cor.EvsT.dif = 0.3, cor.EvsT.abs = -0.3, CP.fc.filter = log2(1.5),CP.p.filter = 0.01, rand = 100, correction = "BH", cores = 1)
{
    ...
    if (dim(CP)[1] == 0) {
		...
		if (sum(index) != 0) {
                  ...
                  tmp <- cbind(tmp, p, fdr)
                  colnames(tmp) <- c("modulator", "effector", "target", "R_low", "R_HIGH","low_index","high_index", "fisrt","last", "P_value", "fdr")
                  MCP <- rbind(MCP, tmp)
                }
            }
            else {
        ...
        colnames(tmptri) <- c("modulator", "effector", "target",
            "R_low", "R_HIGH","low_index","high_index", "first","last", "p-value", "fdr")
        for (tmpi in index) {
